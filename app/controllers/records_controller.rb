...
def index
	...
	if  request.format != "csv"
        @records= Record.search(params[:search]).order(sort_column + " " + sort_direction).paginate(page: params[:page], per_page: 30)
      else

		@records= Record.search(params[:search]).order(sort_column + " " + sort_direction)
